...
n2     = Ytest.shape[0]          # number of testing samples
nt     = Ytrain.shape[0]         # number of atoms oof the dictionary
dmax   = int(np.max(d))          # number of classes

D      = Ytrain  # Dictionary
coder  = SparseCoder(dictionary=D, transform_n_nonzero_coefs=5,transform_algorithm='omp')

ds     = np.zeros((n2,1))        # predcited labels
for i_t in range(n2):
