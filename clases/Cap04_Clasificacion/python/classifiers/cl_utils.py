...
def clf_def(funcName,X,d,par):
    clf = eval(funcName+'('+par+')')
    return clf

def clf_train(funcName,X,d,par):
    clf = clf_def(funcName,X,d,par)
    clf = clf.fit(X,d)
    return clf
...
def clf_train_test(clf_name,X,d,Xt,clf_parm):
    clf = clf_train(clf_name,X,d,clf_parm)
    ds  = clf.predict(Xt)
    return ds,clf

def clf_train_test_ho(clf_name,X,d,Xt,dt,clf_parm):
    ds,clf  = clf_train_test(clf_name,X,d,Xt,clf_parm)
    acc = accuracy_score(dt,ds)     
    return ds,acc,clf

def clf_train_test_cv(clf_name,X,d,clf_parm,folders):
    clf    = clf_def(clf_name,X,d,clf_parm)
...
def clf_best(bcl,X,d,Xt,dt):
    ...
    for i in range(n):
        clf_name = bcl['name'][i]
        clf_parm = bcl['parm'][i]
        ds,acc,_ = clf_train_test_ho(clf_name,X,d,Xt,dt,clf_parm)
        if acc>accmax:
            imax = i
            accmax = acc
            dsbest = ds
...
def clf_best_cv(bcl,X,d,folders):
    ...
    for i in range(n):
        clf_name = bcl['name'][i]
        clf_parm = bcl['parm'][i]
        acc = clf_train_test_cv(clf_name,X,d,clf_parm,folders)
        if acc>accmax:
            imax = i
            accmax = acc
        print(f'     Acc = {acc:.4f}  -> '+clf_name+': '+clf_parm)
...
def clf_bestj(bcl,X,d,Xt,dt):
    ...
    for j in range(m):
        for i in range(n):
            ...
            Xj  = X[:,0:j+1]
            Xtj = Xt[:,0:j+1]
            ds,acc,_ = clf_train_test_ho(clf_name,Xj,d,Xtj,dt,clf_parm)
            if acc>accmax:
                imax = i
                jmax = j
                accmax = acc
